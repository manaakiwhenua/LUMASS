/*=========================================================================
  ...
  Module:    NMVtkQtTableModelAdapter.cxx
...
******************************************************************************/
/*
 * adaptation required because of qt4 to qt5 transition and LUMASS relying on
 * NMVtkQtTableModelAdapter
 *
 */

#include "NMVtkQtTableModelAdapter.h"
...
//----------------------------------------------------------------------------
class NMVtkQtTableModelAdapter::vtkInternal {
public:
...
//----------------------------------------------------------------------------
NMVtkQtTableModelAdapter::NMVtkQtTableModelAdapter(QObject* p)
  : vtkQtAbstractModelAdapter(p)
{
  this->Internal = new vtkInternal;
  this->Table = NULL;
  this->SplitMultiComponentColumns = false;
  this->DecorationLocation = NMVtkQtTableModelAdapter::HEADER;
  this->DecorationStrategy = NMVtkQtTableModelAdapter::NONE;
  this->ColorColumn = -1;
...
//----------------------------------------------------------------------------
NMVtkQtTableModelAdapter::NMVtkQtTableModelAdapter(vtkTable* t, QObject* p)
...
//----------------------------------------------------------------------------
NMVtkQtTableModelAdapter::~NMVtkQtTableModelAdapter()
{
  if (this->Table != NULL)
    {
    this->Table->Delete();
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetColorColumnName(const char* name)
{
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetIconIndexColumnName(const char* name)
{
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetKeyColumnName(const char* name)
{
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetVTKDataObject(vtkDataObject *obj)
{
  vtkTable *t = vtkTable::SafeDownCast(obj);
  if (obj && !t)
    {
    qWarning("NMVtkQtTableModelAdapter needs a vtkTable for SetVTKDataObject");
    return;
    }
...
//----------------------------------------------------------------------------
vtkDataObject* NMVtkQtTableModelAdapter::GetVTKDataObject() const
{
  return this->Table;
}

//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::updateModelColumnHashTables()
{
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::setTable(vtkTable* t)
{
  if (this->Table != NULL)
    {
    this->Table->Delete();
...
//----------------------------------------------------------------------------
bool NMVtkQtTableModelAdapter::noTableCheck() const
{
  if (this->Table == NULL)
    {
...
vtkSelection* NMVtkQtTableModelAdapter::QModelIndexListToVTKIndexSelection(
...
//----------------------------------------------------------------------------
QItemSelection NMVtkQtTableModelAdapter::VTKIndexSelectionToQItemSelection(
...
//----------------------------------------------------------------------------
bool NMVtkQtTableModelAdapter::GetSplitMultiComponentColumns() const
{
  return this->SplitMultiComponentColumns;
}

//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetSplitMultiComponentColumns(bool value)
{
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetDecorationStrategy(int s)
{
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::SetDecorationLocation(int s)
{
...
//----------------------------------------------------------------------------
QVariant NMVtkQtTableModelAdapter::data(const QModelIndex &idx, int role) const
{
  ...
  if (role == Qt::DecorationRole)
    {
    if(this->DecorationStrategy == NMVtkQtTableModelAdapter::COLORS &&
      this->DecorationLocation == NMVtkQtTableModelAdapter::ITEM && this->ColorColumn >= 0)
      {
      return this->getColorIcon(idx.row());
      }
    else if(this->DecorationStrategy == NMVtkQtTableModelAdapter::ICONS &&
      this->DecorationLocation == NMVtkQtTableModelAdapter::ITEM && this->IconIndexColumn >= 0)
      {
...
//----------------------------------------------------------------------------
bool NMVtkQtTableModelAdapter::setData(const QModelIndex &idx, const QVariant &value, int role)
{
  if (role == Qt::DecorationRole)
    {
...
//----------------------------------------------------------------------------
Qt::ItemFlags NMVtkQtTableModelAdapter::flags(const QModelIndex &idx) const
{
  if (!idx.isValid())
    {
...
//----------------------------------------------------------------------------
QVariant NMVtkQtTableModelAdapter::headerData(int section, Qt::Orientation orientation,
                    int role) const
{
  if (this->noTableCheck())
    {
  ...
  if (orientation == Qt::Vertical)
    ...
    if(role == Qt::DisplayRole || role == Qt::UserRole)
      ...
      this->DecorationStrategy == NMVtkQtTableModelAdapter::ICONS &&
      this->DecorationLocation == NMVtkQtTableModelAdapter::ITEM && this->IconIndexColumn >= 0)
      {
...
//----------------------------------------------------------------------------
void NMVtkQtTableModelAdapter::getValue(int row, int in_column, vtkVariant& v) const
{
...
//----------------------------------------------------------------------------
QModelIndex NMVtkQtTableModelAdapter::index(int row, int column,
...
//----------------------------------------------------------------------------
QModelIndex NMVtkQtTableModelAdapter::parent(const QModelIndex & vtkNotUsed(idx)) const
{
  return QModelIndex();
}

//----------------------------------------------------------------------------
int NMVtkQtTableModelAdapter::rowCount(const QModelIndex & mIndex) const
{
  if (this->noTableCheck())
    {
    return 0;
...
//----------------------------------------------------------------------------
int NMVtkQtTableModelAdapter::columnCount(const QModelIndex &) const
{
  if (this->noTableCheck())
    {
    return 0;
...
bool NMVtkQtTableModelAdapter::dropMimeData(const QMimeData *d,
...
QStringList NMVtkQtTableModelAdapter::mimeTypes() const
{
  QStringList types;
  types << "vtk/selection";
  return types;
...
Qt::DropActions NMVtkQtTableModelAdapter::supportedDropActions() const
{
   return Qt::CopyAction;
}

QMimeData *NMVtkQtTableModelAdapter::mimeData(const QModelIndexList &indexes) const
{
...
QVariant NMVtkQtTableModelAdapter::getColorIcon(int row) const
{
  int column;
  if (this->GetSplitMultiComponentColumns())
    {
...
QVariant NMVtkQtTableModelAdapter::getIcon(int row) const
{
  int column;
  if (this->GetSplitMultiComponentColumns())
    {
...
void NMVtkQtTableModelAdapter::SetIconSheet(QImage sheet)
{
  this->IconSheet = sheet;
}
void NMVtkQtTableModelAdapter::SetIconSheetSize(int w, int h)
{
  this->IconSheetSize[0] = w;
  this->IconSheetSize[1] = h;
}
void NMVtkQtTableModelAdapter::SetIconSize(int w, int h)
{
  this->IconSize[0] = w;
  this->IconSize[1] = h;
}
