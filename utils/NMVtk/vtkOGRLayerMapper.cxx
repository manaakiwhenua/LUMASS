 ...
 * Copyright 2010,2011,2012,2012 Landcare Research New Zealand Ltd
 *
...
#ifndef VTK_IMPLEMENT_MESA_CXX
vtkStandardNewMacro(vtkOGRLayerMapper);
#endif
...
vtkOGRLayerMapper::vtkOGRLayerMapper()
{
  this->ListId = 0;
  this->TotalCells = 0;
...
void vtkOGRLayerMapper::ReleaseGraphicsResources(vtkWindow *win)
{
...
//
void vtkOGRLayerMapper::RenderPiece(vtkRenderer *ren, vtkActor *act)
{
...
void vtkOGRLayerMapper::DrawPoints(int idx,
                                         vtkPoints *p, 
                                         vtkDataArray *n,
...
void vtkOGRLayerMapper::DrawLines(int idx,
                                        vtkPoints *p, 
                                        vtkDataArray *n,
                                        vtkUnsignedCharArray *c,
                                        vtkDataArray *t,
...
void vtkOGRLayerMapper::DrawPolygons(int idx,
                                         vtkPoints *p, 
                                         vtkDataArray *n,
                                         vtkUnsignedCharArray *c,
                                         vtkDataArray *t,
...
void vtkOGRLayerMapper::DrawTStrips(int idx,
                                        vtkPoints *p, 
                                        vtkDataArray *n,
                                        vtkUnsignedCharArray *c,
                                        vtkDataArray *t,
...
void vtkOGRLayerMapperDrawTStripLines(int idx,
                                            vtkPoints *p, 
                                            vtkDataArray *n,
                                            vtkUnsignedCharArray *c,
                                            vtkDataArray *t,
...
void vtkOGRLayerMapper::DrawGLUTessPolys(vtkPolyData* data)
{
	NMDebugCtx(ctxOGRLayerMapper, << "...");
	...
	NMDebugAI(<< "drawing " << ncells << " cells" << endl);
	vtkIdType npts;
	vtkIdType cc, pc;

	GLdouble tCoor[3], tRGBA[4];
	std::vector<double*> dp;
	dp.reserve(pts->GetNumberOfPoints());

	// create tesselator
	GLUtesselator* tess = gluNewTess();

	// register callback functions
	gluTessCallback(tess, GLU_TESS_BEGIN, (void (*)())&vtkOGRLayerMapper::beginGLU);
	gluTessCallback(tess, GLU_TESS_VERTEX, (void (*)())&vtkOGRLayerMapper::vertexGLU);
	gluTessCallback(tess, GLU_TESS_END, (void (*)())&vtkOGRLayerMapper::endGLU);
	gluTessCallback(tess, GLU_TESS_COMBINE, (void (*)())&vtkOGRLayerMapper::combineGLU);
	...
	for (cc=0; cc < ncells; cc++)
	{
		...
		for (pc=0; pc < npts; pc++)
		{
			// get memory for holding the data and the temp coord array
			GLdouble* data = new GLdouble[7];
			dp.push_back(data);

			// get the coordinates
			pts->GetPoint(*ptIds, tCoor);
	...
	NMDebugCtx(ctxOGRLayerMapper, << "done!");
}

void vtkOGRLayerMapper::combineGLU(
		  GLdouble coords[3],			// coordinates of new vertex
		  GLdouble* d[4],					// (4 next?) neighbour vertices
		  GLfloat w[4],					// weights for interpolation
		  GLdouble** dataOut)				// new vertex data to be sent to
										// glVertex*()
{
	GLdouble* nv = new GLdouble[7];
	int i,n;
	GLdouble weight, color;

	nv[0] = coords[0];
	nv[1] = coords[1];
	nv[2] = coords[2];

	// interpolate the colour value for the new vertex
	for (i=3; i < 7; i++)
	{
		color = 0;
		// we may have up to four neighbours, but we cannot rely on it!!
		for (n=0; n < 3; n++)
		{
			if (d[n] == NULL)
				break;
...
void vtkOGRLayerMapper::beginGLU(GLenum type)
{
	glBegin(type);
	//NMDebugInd(1, << "glBegin(" << type << ")" << endl);
}

void vtkOGRLayerMapper::vertexGLU(GLvoid* data)
{
	...
	//NMDebugInd(1, << "glVertex3dv(" << ((double*)v)[0] << ", " << ((double*)v)[1] << ", " << v[2] <<
	//		")" << endl);
}

void vtkOGRLayerMapper::endGLU()
{
	glEnd();
	//NMDebugInd(1, << "glEnd()" << endl);
}

void vtkOGRLayerMapper::errorGLU(GLenum errno)
{
	NMErr(ctxOGRLayerMapper, << "errorGLU(" << errno << ")");
}

// Draw method for OpenGL.
int vtkOGRLayerMapper::Draw(vtkRenderer *aren, vtkActor *act)
{
...
//                       GL_POLYGON, input->GetPolys(), ren);
  ...
  if (rep == VTK_POINTS && !prop->GetBackfaceCulling() && !prop->GetFrontfaceCulling() )
    ...
    vtkOGRLayerMapperDrawTStripLines(idx,p,n,c,t,oldCellNum, noAbort,
...
void vtkOGRLayerMapper::PrintSelf(ostream& os, vtkIndent indent)
{
