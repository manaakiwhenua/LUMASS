...
::EnlargeOutputRequestedRegion(DataObject* output)
{
    ...
    //
	////NMDebugAI(<< "requested output region ..." << endl);
	////outputImage->GetRequestedRegion().Print(std::cout, itk::Indent(2));
    //
	//VectorImagePointer offImg = this->m_OffsetImage; //dynamic_cast< VectorImageType * >(this->ProcessObject::GetOutput(1) );
    //
	//if (outputImage.IsNull() || offImg.IsNull())
	//{
	//	NMDebugAI(<< "either output or offset image or both are NULL, so let's quit!" << endl);
	//	return;
	//}
    //
	//// when we're not in 'external streaming' mode, we need the
	//// largest possible region to compute distances;
	//// when we're in 'external streaming' mode, we rely on
	//// suitable regions being provided and suitable parameters
	//// to be set (user's responsibility)
	//if (!this->m_ProcessDownward && !this->m_ProcessUpward)
	//{
	//	outputImage->SetRequestedRegionToLargestPossibleRegion();
	//	offImg->SetRequestedRegionToLargestPossibleRegion();
	//}
...
::GenerateInputRequestedRegion(void)
{
	...
	OutputImageType* out = dynamic_cast<OutputImageType*>(this->GetOutput());
	typename OutputImageType::RegionType lpr = out->GetLargestPossibleRegion();
	typename OutputImageType::RegionType rr = out->GetRequestedRegion();

	if (this->GetInput(0) != 0)
	{
		InputImageType* in = dynamic_cast<InputImageType*>(
				this->ProcessObject::GetInput(0));
		in->SetRequestedRegion(rr);
	}

	if (this->GetInput(1) != 0)
	{
		InputImageType* cost = dynamic_cast<InputImageType*>(
				this->ProcessObject::GetInput(1));
		cost->SetRequestedRegion(rr);
	}
