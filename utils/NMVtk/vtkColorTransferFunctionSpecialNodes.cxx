/*=========================================================================
  ...
  Module:    vtkColorTransferFunctionSpecialNodes.cxx
...
vtkStandardNewMacro(vtkColorTransferFunctionSpecialNodes);
...
class vtkColorTransferFunctionSpecialNodesInternals
{
public:
...
inline void vtkColorTransferFunctionSpecialNodesLabToMsh(const double lab[3], double msh[3])
{
...
inline void vtkColorTransferFunctionSpecialNodesMshToLab(const double msh[3], double lab[3])
{
...
inline double vtkColorTransferFunctionSpecialNodesAngleDiff(double a1, double a2)
{
  double adiff = a1 - a2;
...
inline double vtkColorTransferFunctionSpecialNodesAdjustHue(const double msh[3],
                                                double unsatM)
{
  if (msh[0] >= unsatM - 0.1)
    {
...
inline void vtkColorTransferFunctionSpecialNodesInterpolateDiverging(double s,
                                                         const double rgb1[3],
                                                         const double rgb2[3],
                                                         double result[3])
{
  ...
  vtkColorTransferFunctionSpecialNodesLabToMsh(lab1, msh1);
  vtkColorTransferFunctionSpecialNodesLabToMsh(lab2, msh2);

  // If the endpoints are distinct saturated colors, then place white in between
  // them.
  if (   (msh1[1] > 0.05) && (msh2[1] > 0.05)
      && (vtkColorTransferFunctionSpecialNodesAngleDiff(msh1[2], msh2[2]) > 0.33*vtkMath::DoublePi()) )
    {
  ...
  if ((msh1[1] < 0.05) && (msh2[1] > 0.05))
    {
    msh1[2] = vtkColorTransferFunctionSpecialNodesAdjustHue(msh2, msh1[0]);
    }
  else if ((msh2[1] < 0.05) && (msh1[1] > 0.05))
    {
    msh2[2] = vtkColorTransferFunctionSpecialNodesAdjustHue(msh1, msh2[0]);
    }
  ...
  double labTmp[3];
  vtkColorTransferFunctionSpecialNodesMshToLab(mshTmp, labTmp);
  vtkMath::LabToRGB(labTmp, result);
}

//----------------------------------------------------------------------------
// Construct a new vtkColorTransferFunctionSpecialNodes with default values
vtkColorTransferFunctionSpecialNodes::vtkColorTransferFunctionSpecialNodes()
{
  ...
  this->Internal = new vtkColorTransferFunctionSpecialNodesInternals;
}

//----------------------------------------------------------------------------
// Destruct a vtkColorTransferFunctionSpecialNodes
vtkColorTransferFunctionSpecialNodes::~vtkColorTransferFunctionSpecialNodes()
{
  delete [] this->Table;
...
int vtkColorTransferFunctionSpecialNodes::GetSize()
{
...
double *vtkColorTransferFunctionSpecialNodes::GetDataPointer()
{
...
//----------------------------------------------------------------------------
// Alex's Hack - add a specially treated node to the set of special nodes
void
vtkColorTransferFunctionSpecialNodes::setSpecialNode(int id,
		double value, std::vector<double> rgba)
{
	//if (id < 0 || id > 2)
	//	return;
	//this->mSNV.resize(3);
	this->SpecialValues.push_back(value);
	this->SpecialColours.push_back(rgba);
}

//----------------------------------------------------------------------------
// Add a point defined in RGB
int vtkColorTransferFunctionSpecialNodes::AddRGBPoint( double x, double r,
                                           double g, double b )
{
...
int vtkColorTransferFunctionSpecialNodes::AddHSVPoint( double x, double h,
                                            double s, double v )
{ 
  double r, b, g;
...
// the Range
void vtkColorTransferFunctionSpecialNodes::SortAndUpdateRange()
{
...
//----------------------------------------------------------------------------
bool vtkColorTransferFunctionSpecialNodes::UpdateRange()
{
  double oldRange[2];
...
//----------------------------------------------------------------------------
// Remove a point
int vtkColorTransferFunctionSpecialNodes::RemovePoint( double x )
{
...
//----------------------------------------------------------------------------
void vtkColorTransferFunctionSpecialNodes::MovePoint(double oldX, double newX)
{
  if (oldX == newX)
    {
    // Nothing to do.
...
//----------------------------------------------------------------------------
// Remove all points
void vtkColorTransferFunctionSpecialNodes::RemoveAllPoints()
{
  for(unsigned int i=0;i<this->Internal->Nodes.size();i++)
    {
    delete this->Internal->Nodes[i];
...
void vtkColorTransferFunctionSpecialNodes::AddRGBSegment( double x1, double r1,
                                              double g1, double b1, 
                                              double x2, double r2, 
                                              double g2, double b2 )
{
...
void vtkColorTransferFunctionSpecialNodes::AddHSVSegment( double x1, double h1,
                                              double s1, double v1, 
                                              double x2, double h2, 
                                              double s2, double v2 )
{
...
unsigned char *vtkColorTransferFunctionSpecialNodes::MapValue( double x )
{
  double rgb[3];
  this->GetColor( x, rgb );
  ...
  // Alex, FixMe !
	if (x < this->SpecialValues[1] || x > this->SpecialValues[4])
	  this->UnsignedCharRGBAValue[3] = 0;
	else
...
//-----------------------------------------------------------------------------
// Alex, FixMe! --------------------------------------------
double vtkColorTransferFunctionSpecialNodes::GetOpacity(double value)
{
	double opacity = 1.0;

	if (SpecialValues.size() < 5)
		return opacity;

	if (value < SpecialValues[1] || value > SpecialValues[4])
		opacity = 0.0;
...
void vtkColorTransferFunctionSpecialNodes::GetColor(double x, double rgb[3])
{
...
double vtkColorTransferFunctionSpecialNodes::GetRedValue( double x )
{
  double rgb[3];
  this->GetColor( x, rgb );
...
double vtkColorTransferFunctionSpecialNodes::GetGreenValue( double x )
{
  double rgb[3];
  this->GetColor( x, rgb );
...
double vtkColorTransferFunctionSpecialNodes::GetBlueValue( double x )
{
  double rgb[3];
  this->GetColor( x, rgb );
...
void vtkColorTransferFunctionSpecialNodes::GetTable( double xStart, double xEnd,
    ...
    // Otherwise, we are between two nodes - interpolate
      ...
      if ( sharpness < 0.01 )
        ...
        if ( this->ColorSpace == VTK_CTF_RGB )
          ...
          {
          vtkColorTransferFunctionSpecialNodesInterpolateDiverging(s, rgb1, rgb2, tptr);
          }
        else
          {
...
//----------------------------------------------------------------------------
const unsigned char *vtkColorTransferFunctionSpecialNodes::GetTable( double xStart, double xEnd,
                                                         int size)
{
...
//----------------------------------------------------------------------------
void vtkColorTransferFunctionSpecialNodes::BuildFunctionFromTable(double xStart,
                                                      double xEnd,
                                                      int size,
                                                      double *table)
{
...
int vtkColorTransferFunctionSpecialNodes::GetNodeValue( int index, double val[6] )
{
  int size = static_cast<int>(this->Internal->Nodes.size());
...
//----------------------------------------------------------------------------
// For a specified index value, get the node parameters
int vtkColorTransferFunctionSpecialNodes::SetNodeValue( int index, double val[6] )
{
  int size = static_cast<int>(this->Internal->Nodes.size());
...
//----------------------------------------------------------------------------
void vtkColorTransferFunctionSpecialNodes::DeepCopy( vtkScalarsToColors *o )
{
  vtkColorTransferFunctionSpecialNodes *f = NULL;
  if (o)
    {
    this->Superclass::DeepCopy(o);
    f = vtkColorTransferFunctionSpecialNodes::SafeDownCast(o);
    }
...
//----------------------------------------------------------------------------
void vtkColorTransferFunctionSpecialNodes::ShallowCopy( vtkColorTransferFunctionSpecialNodes *f )
{
  if (f != NULL)
    {
...
// and unsigned short.
template <class T>
void vtkColorTransferFunctionSpecialNodesMapData(vtkColorTransferFunctionSpecialNodes* self,
                                     T* input,
                                     ...
                                     int outFormat, long)
{
  ...
  double minvalue = self->GetRange()[0];
  double maxvalue = self->GetRange()[1];
  double lowermar, lower, upper, uppermar, nd;

  std::vector< std::vector<double> >& sclr = self->getSpecialColours();
  std::vector<double>& sv = self->getSpecialValues();
  ...
  const double low = self->GetRange()[0];
  const double high = self->GetRange()[1];
  while (--i >= 0) 
    {
    x = static_cast<double>(*iptr);
    self->GetColor(x, rgb);

    // special mapping to allow for transparency outside the
    // ramp range (i.e. lower and upper)
    if (special)
    {
    	if (x < lowermar || x > uppermar)
    	{
    		//rgb[0] = sclr[0][0];
    		//rgb[1] = sclr[0][1];
    		//rgb[2] = sclr[0][2];
    		alpha = static_cast<unsigned char>(0);
    	}
    	//else if (x > uppermar)
    	//{
    	//	rgb[0] = sclr[3][0];
    	...
    	//else if (x < lowermar && x >= minvalue)
    	//{
    	//	rgb[0] = sclr[0][0];
    	//	rgb[1] = sclr[0][1];
    	//	rgb[2] = sclr[0][2];
    	//	alpha = static_cast<unsigned char>(sclr[0][3]*255.0);
    	//}
    	//else if (x > upper && x <= uppermar)
    	//{
    	//	rgb[0] = sclr[4][0];
    	//	rgb[1] = sclr[4][1];
    	//	rgb[2] = sclr[4][2];
    	//	alpha = static_cast<unsigned char>(sclr[4][3]*255.0);
    	//}
    	//else if (x > upper && x <= maxvalue)
    	//{
    	//	rgb[0] = sclr[0][0];
    	//	rgb[1] = sclr[0][1];
    	//	rgb[2] = sclr[0][2];
...
//----------------------------------------------------------------------------
void vtkColorTransferFunctionSpecialNodes::MapScalarsThroughTable2(void *input,
                                                       unsigned char *output,
                                                       int inputDataType, 
                                                       int numberOfValues,
                                                       int inputIncrement,
                                                       int outputFormat)
{
  ...
  switch (inputDataType)
    {
    vtkTemplateMacro(
      vtkColorTransferFunctionSpecialNodesMapData(this, static_cast<VTK_TT*>(input),
...
//----------------------------------------------------------------------------
vtkIdType vtkColorTransferFunctionSpecialNodes::GetNumberOfAvailableColors()
{
  if(this->Table)
    {
...
//----------------------------------------------------------------------------
void vtkColorTransferFunctionSpecialNodes::FillFromDataPointer(int nb, double *ptr)
{
  if (nb <= 0 || !ptr)
    {
    return;
...
//----------------------------------------------------------------------------
int vtkColorTransferFunctionSpecialNodes::AdjustRange(double range[2])
{
  if (!range)
    {
    return 0;
...
//----------------------------------------------------------------------------
// Print method for vtkColorTransferFunctionSpecialNodes
void vtkColorTransferFunctionSpecialNodes::PrintSelf(ostream& os, vtkIndent indent)
{
