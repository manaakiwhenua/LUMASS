...
::GDALRATImageFileReader() : otb::ImageFileReader<TOutputImage>(), m_DatasetNumber(0)
{
...
template <class TOutputImage>
void GDALRATImageFileReader<TOutputImage>
...
// have to manually check, whether the current image can be read by GDAL and if so,
  ...
  if (strcmp(this->m_ImageIO->GetNameOfClass(), "GDALRATImageIO") == 0)
    {
    typename GDALRATImageIO::Pointer imageIO = dynamic_cast<GDALRATImageIO*>(this->GetImageIO());
...
::GetAttributeTable(int iBand)
{
	...
	{
		GDALRATImageIO* gio = dynamic_cast<GDALRATImageIO*>(this->m_ImageIO.GetPointer());
		if (gio != 0)
		{
