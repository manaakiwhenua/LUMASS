...
#define GALOG_INFO(arg) {cout arg;}
//\
//    std::lock_guard<std::mutex> log_lock(mtx_log); \
//    MosraLogInfo(arg)
...
public:
	...
	void solve_init()
	{
		...
		if(verbose)
		{
            std::stringstream lcout;

            lcout<<"**************************************"<<endl;
            lcout<<"*             GA started             *"<<endl;
            lcout<<"**************************************"<<endl;
            lcout<<"population: "<<population<<endl;
            lcout<<"elite_count: "<<elite_count<<endl;
            lcout<<"crossover_fraction: "<<crossover_fraction<<endl;
            lcout<<"mutation_rate: "<<mutation_rate<<endl;
            lcout<<"**************************************"<<endl;

            GALOG_INFO(<< lcout.str());
		}
		Chronometer timer;
		timer.tic();
		...
		if(!is_single_objective())
		{
			...
			if(!reference_vector_divisions)
			{
				...
				if(verbose)
				{
                    ...
                    rcout<<"**************************************"<<endl;
                    rcout<<"reference_vector_divisions: "<<reference_vector_divisions<<endl;
                    rcout<<"**************************************"<<endl;
...
protected:
	...
	void show_stop_reason(StopReason stop)
	{
		if(verbose)
		{
            std::stringstream kcout;
            kcout<<"Stop criteria: ";
			if(stop==StopReason::Undefined)
                kcout<<"There is a bug in this function";
			else
                kcout<<stop_reason_to_string(stop);
            kcout<<endl;
            kcout<<"**************************************"<<endl;
            GALOG_INFO(<< kcout.str());
		}
	}
	...
	void select_population_SO(const thisGenerationType &g,thisGenerationType &g2)
	{
        ...
        std::stringstream vcout;
		if(verbose)
            vcout<<"Transfered elites: ";
		vector<int> blocked;
		for(int i=0;i<elite_count;i++)
		{
		...
		if(verbose)
        {
            vcout<<"Selection done."<<endl;
            GALOG_INFO(<< vcout.str());
        }
	}
	...
	****************************************************/
	void init_population(thisGenerationType &generation0)
	{
		...
		if(verbose)
		{
            GALOG_INFO(<<"Initial population of "<<population<<" was created with "<<total_attempts<<" attemps."<<endl);
		}
	}
		...
		for(int index=x_index_begin;index<=x_index_end;index++)
		{
			...
			while(!successful)
			{
				...
				if(pidx_c1==pidx_c2)
					continue ;
				if(verbose)
                    rcout<<"Crossover of chromosomes "<<pidx_c1<<","<<pidx_c2<<endl;
				...
				{
					if(verbose)
                        rcout<<"Mutation of chromosome "<<endl;
            ...
            if (verbose) GALOG_INFO(<< rcout.str());
		}
	...
	void crossover_and_mutation(thisGenerationType &new_generation)
	{
		...
		if(verbose)
		{
            GALOG_INFO(<<"Mutations and crossovers of "<<N_add<<" solutions are calculated with "<<total_attempts<<" attemps."<<endl);
		}
	}
