...
//			this, SLOT(callEditComponentDialog(const QString &)));
    ...
    hFileBox->addWidget(mModelPathEdit);
    ...
    mConfigPathEdit->setPlaceholderText(tr("Model Configuration Source"));
    ...
    mToolContext->addItem("Model's YAML");
...
//    hbox->addSpacing(2);
    ...
    vbox->addItem(hFileBox);
    vbox->addItem(hConfigBox);
...
//            this, SLOT(zoomToComponent(QString)));
    ...
    // animation setup
    mTimeLine = new QTimeLine(1500, this);
    mTimeLine->setFrameRange(0, 100);
    connect(mTimeLine, &QTimeLine::frameChanged, this, &NMModelViewWidget::translateModelView);
}
...
NMModelViewWidget::test()
{
    ...
    auto repo = mModelController->getRepository();
    ...
    while (cpIt != repo.end())
    {
        ...
        NMProcess* proc = procComp->getProcess();
        if (proc == nullptr)
        {
            ++cpIt;
            continue;
                          ...
                          const QPointF& scenePos)
{
    ...
    if (   suffix.compare(QStringLiteral("lmx"), Qt::CaseInsensitive) == 0
        || suffix.compare(QStringLiteral("lmv"), Qt::CaseInsensitive) == 0
       )
    {
        this->loadBenchItems(fileName);
    }
    else if (   suffix.compare(QStringLiteral("yaml"), Qt::CaseInsensitive) == 0
             || suffix.compare(QStringLiteral("yml"), Qt::CaseInsensitive) == 0
            )
    {
        QString ymlTxt = QStringLiteral("YAML File");
        if (mToolContext->findText(ymlTxt) == -1)
        {
            mToolContext->addItem(ymlTxt);
        }
        ...
        QGraphicsItem* item)
{
    ...
    QString _title = "a rouge link";
    if (pi) _title = pi->getTitle();
    else if (ai) _title = ai->getTitle();
    else if (ti) _title = ti->toPlainText();
    else if (wi) _title = wi->objectName();
    else if (li == 0)
    {
        _title = "nothing";
    }
...
NMModelViewWidget::updateToolContextBox(void)
{
    ...
    bool bHasYamlFile = false;
    if (mToolContext->findText(QStringLiteral("YAML File")) >= 0)
    {
        bHasYamlFile = true;
    }
...
bool
NMModelViewWidget::isYamlSequence(const YAML::Node& node)
{
    bool ret = false;
...
QVariant
NMModelViewWidget::parseYamlSetting(const YAML::const_iterator& nit, const QObject* obj)
{
...
void
NMModelViewWidget::configureModel(const YAML::Node& modelConfig)
{
    NMModelController* mController = this->getModelController();
         ...
         if (!compNode.IsNull())
         {
             ...
             while (pit != compNode.end())
             {
                 ...
                 if (bProcProps)
                 {
                     ...
                     {
                         NMLogInfo(<< comp->objectName().toStdString() << ":"
                                   << propName.toStdString()
...
NMModelViewWidget::updateToolContext(const QString &tool)
{
    ...
    if (tool.compare(QStringLiteral("YAML File")) != 0)
    {
        const int yfi = mToolContext->findText(QStringLiteral("YAML File"));
        if (yfi >= 0)
        {
            mModelController->clearModelSettings();
            mConfigPathEdit->clear();
            mToolContext->removeItem(yfi);
        }
    }

    // check whether always present options have been selected
    if (tool.compare(QStringLiteral("None")) == 0)
    {
        mModelController->clearModelSettings();
        mConfigPathEdit->clear();
        mReloadConfigBtn->setEnabled(false);
    ...
    {
        // check whether a tool context was selected
        ...
        else
        {
            ...
            else
            {
                ...
                QString baseName = QString("%1Tool").arg(act->getModelName());
                QString toolTableName = QString("%1/%2.ldb")
                                .arg(act->getModelPath()).arg(baseName);
                mConfigPathEdit->setText(toolTableName);
        ...
        case (qint32)NMAggregateComponentItem::Type:
                {
                    ...
                    title = nameRegister.value(_title);
                    ...
                    // ERROR - couldn't find the model component!!
                    if (ai == nullptr || c == nullptr)
                    {
                        NMLogError(<< "AggregateComponent '" << title.toStdString()
                                   << "' is not a registered model component! "
                                   << "This shouldn't have happened! Make sure "
                                   << "you don't mix lmx with other lmv files!");
                        continue;
                    }
...
NMModelViewWidget::translateModelView(qreal chgval)
{
    ...
    mModelView->centerOn(interCen);
}

void
NMModelViewWidget::focusExecComp(void)
{
...
NMModelViewWidget::processNumIterExprChanges()
{
    ...
    if (exprList.size() > 0)
    {
        foreach(const QString& ex, exprList)
        {
            if (!ex.isEmpty())
            {
                bIsDynamic = true;
    ...
    NMProcessComponentItem* procItem = qgraphicsitem_cast<NMProcessComponentItem*>(gi);
    if (procItem == nullptr)
    {
        return;
    }
    ...
    {
        outLinks.at(l)->setIsDynamic(bIsDynamic);
    }
}
...
void NMModelViewWidget::executeModel(void)
{
    ...
    NMModelComponent* comp = nullptr;
    if (QString("ModelPlayButton").compare(this->sender()->objectName()) == 0)
        comp = this->mModelController->getComponent("root");
    else if (this->mLastItem != 0)
